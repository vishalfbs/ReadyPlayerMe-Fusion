...
public class ThirdPersonCamera : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        distance = maxDistance;//transform.localPosition.magnitude;
    ...
    void Follow_IndependentRotation()
    {
     ...
     //
     // Vector3 desiredCamPos = transform.TransformPoint(dollyDir * maxDistance);//transform.parent.TransformPoint(dollyDir * maxDistance);
     // if (Physics.Linecast(desiredCamPos, transform.position, out hit, mask))//transform.parent.position
     // {
     //     //distance = Mathf.Clamp((hit.distance * 0.9f), minDistance, maxDistance);
     //     distance = hit.distance * 1.2f;
    ...
    {
      //  Debug.Log("camera collider tag name - enter trigger: " + other.name);

        if (other.CompareTag("Player"))
            return;

        //  StopCoroutine(nameof(MoveT));
        //  StartCoroutine(MoveT(Vector3.forward * 2));
        _cameraTransform.localPosition = _cameraTransform.localPosition + Vector3.forward * 2;
    }

    private void OnTriggerExit(Collider other)
    {
      //  Debug.Log("camera collider tag name - exit trigger: " + other.name);

        if (other.CompareTag("Player"))
            return;
        //  StopCoroutine(nameof(MoveT));
        //  StartCoroutine(MoveT(Vector3.zero));
        _cameraTransform.localPosition = Vector3.zero;
    }

 // IEnumerator MoveT(Vector3 _target)
 // {
 //     if (_cameraTransform == null)
 //         yield break;
 //
 //     float dist = Vector3.Distance(_cameraTransform.localPosition, _target);
 //     while (dist > 0.1f)
 //     {
 //         Debug.Log("Dist: " + dist);
 //         dist = Vector3.Distance(_cameraTransform.localPosition, _target);
 //         _cameraTransform.localPosition = Vector3.Lerp(_cameraTransform.localPosition, _target, Time.deltaTime * 10f);
 //         yield return null;
 //     }
 //     _cameraTransform.localPosition = _target;
 // }
}
